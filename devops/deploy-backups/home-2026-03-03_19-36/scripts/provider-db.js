...
export async function getCoverageStats(client, hours = 24) {
  ...
  // Build cumulative vendor count timeline from updated_at so each hour
  // gets the effective total that was active at that time. Uses updated_at
  // (not created_at) because vendors may exist disabled for days then get
  // toggled on — updated_at reflects when they became active.
  const timelineResult = await client.execute(
    "SELECT strftime('%Y-%m-%dT%H:00', updated_at) AS hour, COUNT(*) AS added FROM provider_vendors WHERE enabled = 1 GROUP BY hour ORDER BY hour"
  );
  const sortedTimeline = [];
  let running = 0;
  for (const row of timelineResult.rows) {
    running += Number(row.added);
    sortedTimeline.push({ hour: row.hour, cumulative: running });
  }

  function getEffectiveTotal(hourStr) {
    let effective = 0;
    for (const entry of sortedTimeline) {
      if (entry.hour <= hourStr) effective = entry.cumulative;
      else break;
    }
    return effective || totalEnabled;
  }
  ...
  const hourlyStats = result.rows.map((row) => {
    const effective = getEffectiveTotal(row.hour);
    return {
      hour: row.hour,
