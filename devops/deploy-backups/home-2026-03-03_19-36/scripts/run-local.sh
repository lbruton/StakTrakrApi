...
# ── T1/T2 egress selection ─────────────────────────────────────────────
# T1: route via Tailscale exit node (home residential IP — not blocked by dealers)
# T2: fall through to Fly datacenter IP if Tailscale is unavailable
TS_SOCK="/var/run/tailscale/tailscaled.sock"
TS_EXIT_NODE="${TS_EXIT_NODE:-100.112.198.50}"
if [ -S "$TS_SOCK" ] && tailscale --socket="$TS_SOCK" ping -c 1 --timeout=3s "$TS_EXIT_NODE" >/dev/null 2>&1; then
  tailscale --socket="$TS_SOCK" set --exit-node="$TS_EXIT_NODE" --exit-node-allow-lan-access 2>/dev/null || true
  echo "[$(date -u +%H:%M:%S)] Egress: T1 — Tailscale exit node ($TS_EXIT_NODE)"
else
  tailscale --socket="$TS_SOCK" set --exit-node= 2>/dev/null || true
  echo "[$(date -u +%H:%M:%S)] Egress: T2 — Fly datacenter IP (Tailscale unavailable)"
fi
